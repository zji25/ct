...
public abstract class BinaryOperation implements MSExpression {
    ...
    @Override
    public int evaluate(int x) { return apply(first.evaluate(x), second.evaluate(x)); }

    @Override
    public int evaluate(int x, int y, int z) { return apply(first.evaluate(x, y, z), second.evaluate(x, y, z)); }
    ...
    public String toMiniString() {
        ...
        if (second.priority() < priority() || (second.priority() == priority() && (!isAssociative()
                || !second.isAssociative() && second.priority() == Term.op2priority.get(Term.DIVIDE))))
    ...
    public boolean equals(Object obj) {
        if (obj instanceof BinaryOperation temp) return Objects.equals(getClass(), obj.getClass())
                && Objects.equals(first, temp.first) && Objects.equals(second, temp.second);
        return false;
    }
